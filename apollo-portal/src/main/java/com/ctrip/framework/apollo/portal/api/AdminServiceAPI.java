...
public class AdminServiceAPI
{
	...
	public static class NamespaceAPI extends API
	{
		...
		{
			return restTemplate.post(env, "apps/{appId}/appnamespaces?silentCreation=true", appNamespace,
	...
	public static class ItemAPI extends API
	{
		...
		{
			restTemplate.post(env, "apps/{appId}/clusters/{clusterName}/namespaces/{namespaceName}/itemset", changeSets,
					Void.class, appId, clusterName, namespace);
		}

		public void updateItem(String appId, Env env, String clusterName, String namespace, long itemId, ItemDTO item)
		{
			restTemplate.put(env, "apps/{appId}/clusters/{clusterName}/namespaces/{namespaceName}/items/{itemId}", item,
					appId, clusterName, namespace, itemId);
	...
	public static class ClusterAPI extends API
	{
		...
		{
			return restTemplate.get(env, "apps/{appId}/cluster/{clusterName}/unique", Boolean.class, appId,
					clusterName);
	...
	public static class ReleaseAPI extends API
	{
				...
				String releaseComment, String operator, boolean isEmergencyPublish)
		{
			...
			ReleaseDTO response = restTemplate.post(env,
					"apps/{appId}/clusters/{clusterName}/namespaces/{namespaceName}/releases", entity, ReleaseDTO.class,
					appId, clusterName, namespace);
			return response;
		}
				...
				Set<String> grayDelKeys)
		{
			...
			HttpEntity<MultiValueMap<String, String>> entity = new HttpEntity<>(parameters, headers);
			ReleaseDTO response = restTemplate.post(env,
					"apps/{appId}/clusters/{clusterName}/namespaces/{namespaceName}/gray-del-releases", entity,
				...
				boolean deleteBranch, ItemChangeSets changeSets)
		{
			...
			return restTemplate.post(env,
					...
					changeSets, ReleaseDTO.class, appId, clusterName, namespace, releaseName, releaseComment,
					branchName, deleteBranch, isEmergencyPublish);
	...
	public static class InstanceAPI extends API
	{
		...
		{
			ResponseEntity<PageDTO<InstanceDTO>> entity = restTemplate.get(env,
